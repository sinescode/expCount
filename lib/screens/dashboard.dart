              ...
              sliver: SliverList(
                delegate: SliverChildListDelegate([
                  _BalanceCard(
                    ...
                    todaySpent: p.todaySpent,
                    monthSpent: p.monthSpent,
                    currency: cur,
                    fmt: fmt,
                    settings: p.settings,
                  ),
                        ...
                        child: GlassCard(
                          ...
                          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              ...
                              if (p.overdueDebts.isNotEmpty) ...[
                                ...
                                Container(
                                  ...
                                  child: Text('${p.overdueDebts.length} overdue',
                                      style: const TextStyle(
                                          color: AppTheme.red, fontSize: 9, fontWeight: FontWeight.w700)),
                                ),
                              ],
                            ]),
                            const SizedBox(height: 4),
                            Text('$cur${fmt.format(p.totalOwing)}',
                                style: const TextStyle(
                                    color: AppTheme.red, fontSize: 18, fontWeight: FontWeight.w700)),
                          ]),
                        ),
                      ),
                    ]),
...
class _BalanceCard extends StatelessWidget {
  ...
  const _BalanceCard({
    required this.balance, required this.todaySpent, required this.monthSpent,
    required this.currency, required this.fmt, required this.settings,
  });
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          _stat('Today', '$currency${fmt.format(todaySpent)}', AppTheme.red),
          const SizedBox(width: 24),
          _stat('This Month', '$currency${fmt.format(monthSpent)}', AppTheme.orange),
        ]),
        if (budgetUsage != null) ...[
          const SizedBox(height: 14),
...
class _QuickActions extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final actions = [
      ('💸', 'Expense', () => Navigator.push(context,
          MaterialPageRoute(builder: (_) => const AddTransactionScreen()))),
      ('💰', 'Income', () => Navigator.push(context,
          MaterialPageRoute(builder: (_) =>
              const AddTransactionScreen(initialType: TransactionType.income)))),
      ('🔒', 'Hidden', () => Navigator.push(context,
          MaterialPageRoute(builder: (_) => const VaultScreen()))),
      ('🤝', 'Debt', () => Navigator.push(context,
          MaterialPageRoute(builder: (_) => const DueScreen()))),
    ];
              ...
              barRods: [
                BarChartRodData(
                  ...
                  gradient: isToday
                      ? AppTheme.accentGrad
                      : LinearGradient(
                          colors: [AppTheme.accent.withOpacity(0.5), AppTheme.accent.withOpacity(0.3)],
        ...
        child: Column(children: [
          ...
          TextField(
            ...
            decoration: const InputDecoration(
                hintText: 'Search transactions...', prefixIcon: Icon(Icons.search, size: 18)),
            onChanged: (q) => setState(() => _results = widget.provider.searchTransactions(q)),
          ),
          const SizedBox(height: 12),
          Expanded(
            child: _results.isEmpty && _ctrl.text.isNotEmpty
                ? const EmptyState(icon: Icons.search_off, message: 'No results found')
                : ListView.separated(
                    controller: scroll,
