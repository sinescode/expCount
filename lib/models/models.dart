...
extension TransactionCategoryExt on TransactionCategory {
  ...
  IconData get icon {
    switch (this) {
      ...
      case TransactionCategory.work:          return Icons.work;
      case TransactionCategory.travel:        return Icons.flight;
      case TransactionCategory.other:         return Icons.category;
    }
  }
}
...
class Transaction {
  ...
  factory Transaction.fromJson(Map<String, dynamic> j, {VaultCrypto? crypto}) {
    ...
    return Transaction(
      id: j['id'],
      title: dec('title', 'Hidden Entry'),
      amount: (j['amount'] as num).toDouble(),
      type: TransactionType.values.firstWhere(
        (e) => e.name == dec('type', 'expense'),
        orElse: () => TransactionType.expense,
      ),
      category: TransactionCategory.values.firstWhere(
        (e) => e.name == dec('category', 'other'),
        orElse: () => TransactionCategory.other,
      ),
      dateTime: DateTime.tryParse(dec('dateTime', '')) ?? DateTime.now(),
      isHidden: j['isHidden'] ?? false,
      note: decOpt('note'),
      tag: decOpt('tag'),
      isRecurring: j['isRecurring'] ?? false,
      paymentMethod: decOpt('paymentMethod'),
    );
  }
}
...
class Debt {
  ...
  bool get isOverdue =>
      dueDate != null && dueDate!.isBefore(DateTime.now()) && status != DebtStatus.settled;
  ...
  factory Debt.fromJson(Map<String, dynamic> j, {VaultCrypto? crypto}) {
    ...
    return Debt(
      id: j['id'],
      personName: dec('personName', 'Hidden'),
...
class Reminder {
  ...
  Reminder copyWith({bool? isActive}) => Reminder(
    ...
    isActive: isActive ?? this.isActive, linkedDebtId: linkedDebtId,
  );

  Map<String, dynamic> toJson() => {
    'id': id, 'title': title, 'note': note,
    'dateTime': dateTime.toIso8601String(),
    'isActive': isActive, 'linkedDebtId': linkedDebtId,
  };

  factory Reminder.fromJson(Map<String, dynamic> j) => Reminder(
    id: j['id'], title: j['title'], note: j['note'],
    dateTime: DateTime.parse(j['dateTime']),
    isActive: j['isActive'] ?? true, linkedDebtId: j['linkedDebtId'],
  );
}
...
class AppSettings {
  ...
  const AppSettings({
    this.currency = '৳',       // BDT Taka default
    this.darkMode = true,
  ...
  AppSettings copyWith({
    String? currency, bool? darkMode, bool? appLockEnabled,
    String? pin, bool? biometricEnabled,
  ...
  Map<String, dynamic> toJson() => {
    'currency': currency, 'darkMode': darkMode,
    'appLockEnabled': appLockEnabled, 'pin': pin,
    'biometricEnabled': biometricEnabled,
    'monthlyBudget': monthlyBudget, 'dailyBudget': dailyBudget,
  };
